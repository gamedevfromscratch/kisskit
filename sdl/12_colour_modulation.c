...
char *title = "SDL 12 - Colour Modulation";
SDL_Window *window;
...
int sprite_draw_coloured(SDL_Rect *sprite, int x, int y, SDL_Color *c)
{
	int status = -1;
...
#define SPRITE_DRAW_COLOURED(...) SDL_ZERO(sprite_draw_coloured, __VA_ARGS__)
...
int draw(void)
{
	...
	SPRITE_DRAW_COLOURED(&sprite[0], x, y, &tint);
	SPRITE_DRAW_COLOURED(&sprite[1], w - sprite[1].w - x, y, &tint);
	SPRITE_DRAW_COLOURED(&sprite[2], x, h - sprite[2].h - y, &tint);
	SPRITE_DRAW_COLOURED(&sprite[3], w - sprite[3].w - x, h - sprite[3].h - y, &tint);
