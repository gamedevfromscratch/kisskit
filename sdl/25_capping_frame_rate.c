...
char *title = "SDL 25 - Capping Frame Rate";
SDL_Window *window;
...
enum { target_fps = 60, cap_ticks_per_frame = 1000 / target_fps };
...
struct timer _cap_timer = { 0, 0 };

struct timer *cap_timer = &_cap_timer;
...
int main(void)
{
	...
	while (!quit) {
		...
		frame_ticks = timer_get(cap_timer);
		if (frame_ticks < cap_ticks_per_frame)
			SDL_Delay(cap_ticks_per_frame - frame_ticks);
		n_frames++;
